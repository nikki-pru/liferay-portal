...
test.describe('Localizable Configuration', () => {
	...
	test(
		'Can configure the editables added by a localizable configuration value in a translated language',
		{tag: '@LPD-105701'},
		async ({apiHelpers, page, pageEditorPage, site}) => {

			// Import a fragment whose editables are generated by a localizable
			// configuration field
			...
			const html = `
				<div class="fragment-example">
					[#list 1..configuration.numberOfLinks as i]
						<a
							data-lfr-editable-id="link-\${i}"
			...
			// The default language keeps the default value of 1 link

			await expect(page.locator('.fragment-example a')).toHaveCount(1);

			// Set the number of links to 3 in the translated language
			...
			await expect(page.locator('.fragment-example a')).toHaveCount(3);

			// Publish the page and open the editor again
			...
			// Open the page structure and switch the language with it open

			await page.getByRole('tab', {exact: true, name: 'Browser'}).click();

			await page
				.locator(
					`[data-title="${fragmentEntryName}"] .component-expander`
				)
				.first()
				.click();
			...
			// The editables that only exist in the translated language keep
			// their own type

			await page
				.getByRole('button', {exact: true, name: 'Select link-2'})
				.click();

			await expect(
				page.getByRole('tab', {exact: true, name: 'Link'})
			).toBeVisible();
		}
	);
});
